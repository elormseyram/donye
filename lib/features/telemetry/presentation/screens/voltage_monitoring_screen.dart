...
class VoltageMonitoringScreen extends ConsumerWidget {
  const VoltageMonitoringScreen({super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Voltage')),
      body: ListView(
        padding: const EdgeInsets.all(AppSpacing.lg),
        children: [
          Center(
            child: TelemetryGauge(
              value: telemetry?.voltageV ?? 0,
              min: 0,
              max: 84,
              label: 'Voltage',
              unit: 'V',
                  ...
                  child: historyAsync.when(
                    ...
                    data: (history) {
                      ...
                      final spots = sorted.asMap().entries.map((e) {
                        return FlSpot(e.key.toDouble(), e.value.voltageV);
                      }).toList();
                      return LineChart(
                        LineChartData(
